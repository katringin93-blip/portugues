export default {
  title: "Минулий незавершений час (Imperfeito)",
  description: "Pretérito Imperfeito — опис минулих звичок, станів та тривалих дій",
  theory: [
    {
      heading: "Pretérito Imperfeito",
      text: "Цей час використовується для опису: 1) звичних дій у минулому, 2) фонових/тривалих дій, 3) опису обстановки та станів у минулому, 4) одночасних дій у минулому. Еквівалент українського недоконаного виду: «я робив», «він ходив»."
    },
    {
      heading: "Правильні дієслова",
      text: "В Imperfeito дієслова відмінюються дуже регулярно. Неправильних дієслів усього чотири."
    },
    {
      table: {
        headers: ["Особа", "-ar (falar)", "-er (comer)", "-ir (partir)"],
        rows: [
          ["eu", "falava", "comia", "partia"],
          ["tu", "falavas", "comias", "partias"],
          ["ele/ela/você", "falava", "comia", "partia"],
          ["nós", "falávamos", "comíamos", "partíamos"],
          ["eles/elas/vocês", "falavam", "comiam", "partiam"]
        ]
      }
    },
    {
      heading: "Неправильні дієслова в Imperfeito",
      text: "Лише чотири дієслова неправильні в цьому часі: ser, ter, vir, pôr."
    },
    {
      table: {
        headers: ["Особа", "ser", "ter", "vir", "pôr"],
        rows: [
          ["eu", "era", "tinha", "vinha", "punha"],
          ["tu", "eras", "tinhas", "vinhas", "punhas"],
          ["ele/ela", "era", "tinha", "vinha", "punha"],
          ["nós", "éramos", "tínhamos", "vínhamos", "púnhamos"],
          ["eles/elas", "eram", "tinham", "vinham", "punham"]
        ]
      }
    },
    {
      note: "Важливо: дієслова dar, dizer, fazer, ir, saber, trazer, ver — неправильні в інших часах, але в Imperfeito вони відмінюються за звичайними правилами: dava, dizia, fazia, ia, sabia, trazia, via."
    },
    {
      text: "Примітка: у деяких граматиках pôr відносять до 4-ї групи відмінювання поряд із -ar/-er/-ir. В Imperfeito дієслово pôr відмінюється за власною схемою (punha, punhas...), але ця схема цілком передбачувана."
    },
    {
      heading: "Приклади з ser, ter, vir в Imperfeito",
      text: "Приклади з підручника, що ілюструють вживання неправильних дієслів ser, ter та vir в Imperfeito."
    },
    {
      examples: [
        { pt: "O Fernando era muito gordo.", ru: "Фернандо був дуже повним." },
        { pt: "No ano passado tínhamos um automóvel muito velho.", ru: "Минулого року у нас був дуже старий автомобіль." },
        { pt: "O meu tio tinha uma namorada alemã. Era muito simpática.", ru: "У мого дядька була подруга-німкеня. Вона була дуже симпатичною." },
        { pt: "Vinhas a nossa casa muitas vezes, quando vivias aqui perto.", ru: "Ти часто приходив до нас, коли жив тут поруч." }
      ]
    },
    {
      heading: "Маркери Imperfeito",
      text: "Слова-маркери, які вказують на використання Imperfeito."
    },
    {
      table: {
        headers: ["Маркер", "Переклад"],
        rows: [
          ["sempre", "завжди"],
          ["todos os dias", "щодня"],
          ["normalmente", "зазвичай"],
          ["antigamente", "раніше, за старих часів"],
          ["quando era criança", "коли я був дитиною"],
          ["naquela época", "у ту епоху"],
          ["enquanto", "тоді як, поки"]
        ]
      }
    },
    {
      examples: [
        { pt: "Quando era criança, brincava no jardim.", ru: "Коли я був дитиною, я грався в саду." },
        { pt: "Todos os dias ia à escola a pé.", ru: "Щодня я ходив до школи пішки." },
        { pt: "Enquanto eu estudava, ele via televisão.", ru: "Поки я вчився, він дивився телевізор." },
        { pt: "Naquela época, vivíamos no campo.", ru: "У ту епоху ми жили в селі." }
      ]
    },
    {
      note: "Imperfeito — один із найзручніших часів, оскільки майже всі дієслова правильні (окрім ser, ter, vir, pôr). Форми 1-ї та 3-ї особи однини збігаються."
    },
    {
      heading: "Ir + інфінітив в Imperfeito: «збирався зробити»",
      text: "Дієслово ir в Imperfeito + інфінітив основного дієслова описує дію, яку «збиралися зробити» в минулому (аналог англійського «was going to»)."
    },
    {
      examples: [
        { pt: "Eu ia telefonar à Ana, mas depois esqueci-me.", ru: "Я збирався зателефонувати Ані, але потім забув." },
        { pt: "Ela ia dar aulas este ano, mas desistiu.", ru: "Вона збиралася вести уроки цього року, але відмовилась." },
        { pt: "Nós íamos mudar de casa, mas mudámos de ideias.", ru: "Ми збиралися переїхати, але передумали." }
      ]
    },
    {
      heading: "Ввічливі прохання з Imperfeito",
      text: "Imperfeito використовується для ввічливих прохань та побажань, пом'якшуючи пряме висловлювання (аналог українського умовного «хотів би», «міг би»)."
    },
    {
      examples: [
        { pt: "Eu queria um café, por favor.", ru: "Я хотів би каву, будь ласка." },
        { pt: "Joana, podias fazer-me um favor?", ru: "Жуана, ти не могла б мені допомогти?" },
        { pt: "O senhor podia dizer-me as horas?", ru: "Ви не могли б підказати час?" }
      ]
    }
  ],
  lessons: [
    { title: "Частина 1: Imperfeito — форми і маркери" },
    { title: "Частина 2: Ir + інфінітив і ввічливі прохання" }
  ],
  vocabulary: [
    { ru: "раніше, за колишніх часів" },
    { ru: "у ту епоху" },
    { ru: "щодня" },
    { ru: "зазвичай" },
    { ru: "поки, тоді як" },
    { ru: "гратися (дитяча гра)" },
    { ru: "сад" },
    { ru: "село; поле" },
    { ru: "дитина" },
    { ru: "телевізор, телебачення" },
    { ru: "курорт, місце відпочинку" },
    { ru: "мікроклімати" },
    { ru: "що походять з" },
    { ru: "у підлітковому віці" },
    { ru: "весь, усе" },
    { ru: "зразок тканини" }
  ],
  flashcards: [
    { ru: "говорив (звично)", hint: "Imperfeito від falar" },
    { ru: "був (постійно)", hint: "Imperfeito від ser (неправ.)" },
    { ru: "мав", hint: "Imperfeito від ter (неправ.)" },
    { ru: "приходив", hint: "Imperfeito від vir (неправ.)" },
    { ru: "раніше", hint: "маркер Imperfeito" },
    { ru: "поки, тоді як", hint: "сполучник, маркер Imperfeito" },
    { ru: "гратися", hint: "eu brincava (Imperf.)" },
    { ru: "дитина", hint: "мн.: as crianças" },
    { ru: "відкрити для публіки", hint: "вираз" },
    { ru: "несправний, зламаний", hint: "прикметник" },
    { ru: "шумний", hint: "прикметник" },
    { ru: "шум", hint: "ч.р." },
    { ru: "контекст", hint: "ч.р." },
    { ru: "різниця, відмінність", hint: "ж.р." },
    { ru: "двоїстість", hint: "ж.р." },
    { ru: "неправильний, помилковий", hint: "прикметник" },
    { ru: "непосидючий, неспокійний", hint: "прикметник" },
    { ru: "ліцей, гімназія", hint: "ч.р." },
    { ru: "плани", hint: "ч.р., мн." },
    { ru: "годинник", hint: "ч.р." },
    { ru: "решта", hint: "прикметник" },
  ],
  exercises: [
    // Вправа 1: Перепишіть в Imperfeito (0-9)
    { question: "Перепишіть в Imperfeito: Eles não sabem a resposta. → Eles não ___" },
    { question: "Перепишіть в Imperfeito: Ela quer ir embora. → Ela ___" },
    { question: "Перепишіть в Imperfeito: Tu ouves música e fazes exercício. → Tu ___ música e ___ exercício." },
    { question: "Перепишіть в Imperfeito: Nós vivemos em Lisboa e viajamos muito. → Nós ___ em Lisboa e ___ muito." },
    { question: "Перепишіть в Imperfeito: Eles nunca têm tempo. → Eles nunca ___" },
    { question: "Перепишіть в Imperfeito: Tu adoras dançar. → Tu ___" },
    { question: "Перепишіть в Imperfeito: Tu tens tempo e chegas cedo. → Tu ___ tempo e ___ cedo." },
    { question: "Перепишіть в Imperfeito: Eles pensam muito. → Eles ___" },
    { question: "Перепишіть в Imperfeito: Nós fazemos tudo e pomos a mesa. → Nós ___ tudo e ___ a mesa." },
    { question: "Перепишіть в Imperfeito: Eles sabem a verdade. → Eles ___" },
    // Вправа 2: Заповніть Imperfeito (10-21) — португальські підказки, нічого не перекладати
    { question: "Провідміняйте в Imperfeito: O professor ___ muito bem. (ensinar)" },
    { question: "Провідміняйте в Imperfeito: Tu ___ ao cinema todos os sábados. (ir)" },
    { question: "Провідміняйте в Imperfeito: Eu não ___ a resposta. (saber)" },
    { question: "Провідміняйте в Imperfeito: Ele ___ toda a gente na cidade. (conhecer)" },
    { question: "Провідміняйте в Imperfeito: Ela ___ sempre cansada. (estar)" },
    { question: "Провідміняйте в Imperfeito: O João ___ muito inteligente. (ser)" },
    { question: "Провідміняйте в Imperfeito: Ninguém ___ o que fazer. (saber)" },
    { question: "Провідміняйте в Imperfeito: Ela ___ mudar de emprego. (querer)" },
    { question: "Провідміняйте в Imperfeito: Os dias ___ mais curtos no inverno. (ser)" },
    { question: "Провідміняйте в Imperfeito: O bebé ___ muito durante o dia. (dormir)" },
    { question: "Провідміняйте в Imperfeito: O meu avô ___ na fábrica. (trabalhar)" },
    { question: "Провідміняйте в Imperfeito: Eu ___ muito de chocolate. (gostar)" },
    // Вправа 3: Перекладіть португальською (22-29)
    { source: "Раніше ми жили в маленькому будинку." },
    { source: "Коли я був дитиною, я грався в парку." },
    { source: "Вона завжди носила червону сукню." },
    { source: "Кожного дня він вставав о шостій ранку." },
    { source: "Погода була чудовою." },
    { source: "Ми не знали його імені." },
    { source: "Вони хотіли подорожувати Європою." },
    { source: "Вулиці були тихими й темними." },
    // Вправа 4: Діалог (30-43)
    { question: "Заповніть діалог (Imperfeito): — Como é que tu ___ no antigo trabalho? (estar)", explanation: "З «tu» в Imperfeito: estavas. У цьому контексті також допускається «estava» (розмовна форма)." },
    { question: "Заповніть діалог (Imperfeito): — O trabalho ___ muito. (cansar)" },
    { question: "Заповніть діалог (Imperfeito): — O que os colegas ___ durante o almoço? (fazer)" },
    { question: "Заповніть діалог (Imperfeito): — Eu não ___ muito tempo livre. (ter)" },
    { question: "Заповніть діалог (Imperfeito): — Tu ___ mudar de emprego? (querer)" },
    { question: "Заповніть діалог (Imperfeito): — Eu ___ do ambiente de trabalho. (gostar)" },
    { question: "Заповніть діалог (Imperfeito): — Tu ___ a pé para o escritório? (ir)" },
    { question: "Заповніть діалог (Imperfeito): — Tu ___ muitas horas? (trabalhar)" },
    { question: "Заповніть діалог (Imperfeito): — O chefe ___ exigente? (ser)" },
    { question: "Заповніть діалог (Imperfeito): — Ele ___ a equipa com rigor. (gerir)" },
    { question: "Заповніть діалог (Imperfeito): — Tu ___-te bem lá? (sentir)" },
    { question: "Заповніть діалог (Imperfeito): — Eu ___ ter mais liberdade. (querer)" },
    { question: "Заповніть діалог (Imperfeito): — Tu ___ os colegas frequentemente? (ver)" },
    { question: "Заповніть діалог (Imperfeito): — Ele ___ com muita gente. (falar)" },
    // Вправа 5: Перекладіть португальською (44-53)
    { source: "Мій дідусь завжди читав газету вранці." },
    { source: "Коли я був молодим, я багато подорожував." },
    { source: "Вона працювала в лікарні." },
    { source: "Ми ходили до школи щодня." },
    { source: "Діти гралися у дворі." },
    { source: "Він не хотів вчитися." },
    { source: "Раніше не було інтернету." },
    { source: "У неї було багато друзів." },
    { source: "Кожного літа ми їздили на пляж." },
    { source: "Він завжди приходив пізно." },
    // Вправа 6: Історичний текст (54-63) — португальські підказки, нічого не перекладати
    { question: "Заповніть історичний текст (Imperfeito): No início do século XX, Portugal ___ um país rural. (ser)" },
    { question: "Заповніть історичний текст (Imperfeito): As condições de vida ___ difíceis. (ser)" },
    { question: "Заповніть історичний текст (Imperfeito): O povo ___ ___ oprimido. (sentir-se)" },
    { question: "Заповніть історичний текст (Imperfeito): O país ___ muitos problemas económicos. (ter)" },
    { question: "Заповніть історичний текст (Imperfeito): A população ___ ___ com o campo. (identificar-se)" },
    { question: "Заповніть історичний текст (Imperfeito): Não ___ muita indústria. (existir)" },
    { question: "Заповніть історичний текст (Imperfeito): As famílias ___-___ com dificuldades diárias. (confrontar-se)" },
    { question: "Заповніть історичний текст (Imperfeito): A pobreza ___ muitos a emigrar. (levar)" },
    { question: "Заповніть історичний текст (Imperfeito): As viagens ___-___ longas e perigosas. (tornar-se)" },
    { question: "Заповніть історичний текст (Imperfeito): O Brasil ___-___ como destino principal. (destacar-se)" },
    // Додаткові вправи (64-69)
    { question: "Зіставте неправильне дієслово з формою «eu» в Imperfeito:" },
    { question: "Складіть речення: «Раніше він працював у банку.»" },
    {
      question: "Скільки неправильних дієслів в Imperfeito?",
      options: ["Два (ser, ir)", "Три (ser, ter, vir)", "Чотири (ser, ter, vir, pôr)", "П'ять"],
      explanation: "В Imperfeito рівно чотири неправильних дієслова: ser, ter, vir, pôr."
    },
    {
      question: "В Imperfeito форми 1-ї та 3-ї особи однини...",
      options: [
        "завжди різняться",
        "збігаються",
        "різняться лише у неправильних дієслів",
        "різняться лише у дієслів на -ar"
      ],
      explanation: "В Imperfeito форми «eu» і «ele/ela/você» завжди збігаються: eu falava = ele falava."
    },
    {
      question: "Зіставте маркери Imperfeito з перекладом:",
      pairs: [
        { left: "sempre", right: "завжди" },
        { left: "todos os dias", right: "щодня" },
        { left: "normalmente", right: "зазвичай" },
        { left: "antigamente", right: "раніше, за старих часів" },
        { left: "enquanto", right: "поки, тоді як" }
      ]
    },
    { question: "Складіть речення: «Діти зазвичай гралися в саду.»" }
  ],

};
